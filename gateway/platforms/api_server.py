...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _write_sse_responses(
        ...
        If the client disconnects mid-stream and ``store=True``, the agent is
        allowed to continue running so the completed response is persisted to
        ``ResponseStore`` and can be recovered via
        ``GET /v1/responses/{response_id}``.  For ``store=False`` the legacy
        behavior is preserved: ``agent.interrupt()`` is called on disconnect so
        the agent stops issuing upstream LLM calls and the asyncio task is
        cancelled — no token spend on a response no one will ever read.
        If the server-side task itself is cancelled (for example during
        shutdown), an ``incomplete`` snapshot is persisted so
        ``GET /v1/responses/{response_id}`` and ``previous_response_id``
        chaining still have something to recover from.
        """
        import queue as _q
        ...
        try:
            ...
            async def _dispatch(it) -> None:
                ...
                if isinstance(it, tuple) and len(it) == 2 and isinstance(it[0], str):
                    ...
                    if not store:
                        # For ephemeral streams, write synchronously so a client
                        # disconnect propagates to the outer handler, which
                        # interrupts/cancels the agent to avoid wasting tokens.
